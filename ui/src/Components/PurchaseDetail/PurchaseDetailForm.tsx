...
const PurchaseDetail: React.FC = () => {
    const [PurchaseDetailModalOpen, PurchaseDetailSetIsModalOpen] = useState(false);
    const [PurchaseDetailItems, PurchaseDetailSetItems] = useState<ItemFormValues[]>([]);
    const [PurchaseDetailEditingIndex, PurchaseDetailSetEditingIndex] = useState<number | null>(null);
    ...
    return (
        <div style={{ marginTop: 20 }}>
            <Card
                title="Item Details"
                extra={
                    <Button type="primary" onClick={() => PurchaseDetailSetIsModalOpen(true)}>
                        Add Item
                    </Button>
                }
            >
...
const PurchaseDetailsForm: React.FC<PurchaseDetailsFormProps> = ({ form }) => {
    ...
    const renderDatePicker = (name: keyof PurchaseFormValues) => {
        const value = activeForm.getFieldValue(name);
        return (
            <DatePicker
                style={{ width: "100%" }}
                value={value || null}
                onChange={(d) => activeForm.setFieldsValue({ [name]: d })}
            />
        );
    };
    ...
    return (
        <>
            <Form layout="vertical" form={activeForm} onFinish={handleSubmit}>
                {/* Row 1 */}
                <Row gutter={16}>
                    <Col span={8}>
