...
interface Props {
  visible: boolean;
  editingData?: ItemFormValues | null;   
  onClose: () => void;
  onSubmit: (values: ItemFormValues) => void;
}

const PurchaseItemModal: React.FC<Props> = ({
  visible,
  editingData = null,
  onClose,
  onSubmit,
  ...
  return (
    <Modal
      title={editingData ? "Edit Purchase Item" : "Add Purchase Item"}  
      open={visible}
      onCancel={onClose}
      onOk={handleSubmit}
      okText={editingData ? "Update" : "Save"}  
      width={520}
      centered
      bodyStyle={{
        padding: "16px 20px",
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Total Qty */}
          <Form.Item name="totalQty" label="Total Quantity">
            <InputNumber style={{ width: "100%" }} disabled />
          </Form.Item>

          {/* Price per Unit */}
          <Form.Item name="pricePerUnit" label="Price" rules={[{ required: true }]}>
            <InputNumber min={0} style={{ width: "100%" }} />
          </Form.Item>
