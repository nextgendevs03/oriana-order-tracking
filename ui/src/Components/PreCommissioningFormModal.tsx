...
const PreCommissioningFormModal: React.FC<PreCommissioningFormModalProps> = ({
  ...
  const serialOptions = useMemo((): SerialOption[] => {
    ...
    const existingSerials = preCommissioningDetails
      .filter((pc) => pc.poId === poId && pc.id !== editData?.id)
      .map((pc) => pc.serialNumber);
  ...
  useEffect(() => {
    if (visible && editData) {
      ...
      if (editData.ppmDocuments && editData.ppmDocuments.length > 0) {
        const existingFiles: UploadFile[] = editData.ppmDocuments.map((doc) => ({
          uid: doc.uid,
          name: doc.name,
          status: "done",
          url: doc.url,
      ...
      form.setFieldsValue({
        serialNumbers: [`${editData.dispatchId}__${editData.product}__${editData.serialNumber}`],
        ...
        <Row gutter={24}>
          <Col span={24}>
            <Form.Item name="oemComments" label="Comment from OEM">
              <Input.TextArea placeholder="Enter comments from OEM (optional)" rows={2} />
            </Form.Item>
          </Col>
        </Row>
